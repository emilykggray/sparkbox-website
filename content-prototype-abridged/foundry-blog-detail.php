...
<body>
	...
	<section class="listing blog">
		<article class="post">
		 ...
		 <p>On a recent project,&nbsp;Jeremy handed me a design that included some hip, slanted toggles in the responsive navigation for smaller screen sizes (similar to the image above). My first thought was to simply drop in a <code>background-image</code> and go about my merry way. In actuality, getting a correct implementation was a little more challenging. The final solution removes the need for any image requests, and it maintains perfect target accuracy for touch devices. Here's how we got there.</p>
		 <h3>First Try</h3>
		 <p>My first real attempt at implementing the slants was to use <code>:before</code> and <code>:after</code> pseudo classes and use background images to create the illusion of a slanted background. The main issue with this is that the click targets were all wrong. Clicking in the upper right or lower left corners of these faux slants resulted in a click on the element next to the targeted element. Also, this approach made four total requests for two toggles which obviously isn't winning any performance awards.</p>
		 <h3>Rotation Inception</h3>
		 <p>After talking it over with Ben, he recommended trying to use <code>transform: rotate</code> on the elements. I had tried this before and was disappointed because the children elements also rotated (in this case, the text inside the toggle). At Ben's urging, I gave it another go. After some Googling and thinking, I figured out that simply applying absolute positioning will not remove the parent's rotation, but it will allow a second transform (a counter-rotate) to transform the child element. One issue I ran into with this approach was that elements had to be made larger than desired and then cropped with&nbsp;<code>overflow:hidden</code>&nbsp; with their parent.</p>
		 <h3>The Results</h3>
		 <p>The elements were successfully rotated, their children counter-rotated, no images were loaded, and (most importantly) the rotate also changed the element's touch target to correspond directly to its visual appearance. This is a big change from a solution that<span style="font-size: 12px;">&nbsp;had required one to four images, one to four requests, and resulted in an inaccurate target.&nbsp;</span><span style="font-size: 12px;">Pretty sweet, huh?</span></p>
		 <p>WebKit<span style="font-size: 12px;"> obviously handles this fine, as does Firefox. According to </span><a href="http://caniuse.com/#search=transform" style="font-size: 12px;">Can I Use</a>,<span style="font-size: 12px;"> IE 9+ will handle the transforms just fine as well. We thought this was an acceptable level of browser support. For </span>IE8<span style="font-size: 12px;"> and lower we decided it wasn't vital that they receive the slanted treatment, so they instead get standard square boxes.</span></p>
